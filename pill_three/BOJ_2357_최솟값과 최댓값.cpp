...
ll max_init(vector<ll>& v, vector<ll>& tree, int node, int start, int end) {
	if (start == end) {
		return tree[node] = v[start];
	}
	else {
		return tree[node] = max(max_init(v, tree, node * 2, start, (start + end) / 2), max_init(v, tree, node * 2 + 1, (start + end) / 2 + 1, end));
	}
}
